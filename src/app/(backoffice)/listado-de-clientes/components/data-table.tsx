...
import { deleteCustomer } from '@/app/(backoffice)/listado-de-clientes/actions'
import type {
  ColumnsDefinition,
  CustomerColumn
} from '@/app/(backoffice)/listado-de-clientes/components/columns'
...
export function DataTable({
  ...
  const [columnVisibility, setColumnVisibility] = useState<VisibilityState>(
    () => {
      ...
      if (columnsArray.length > 0) {
        columns.forEach((column: CustomerColumn) => {
  ...
  const table = useReactTable({
    data,
    columns: columns.map((column: CustomerColumn) => {
  ...
  return (
    <div className="flex h-full flex-col">
      <TableControls<Customer> table={table} />
